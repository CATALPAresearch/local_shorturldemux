...
if (isset($_GET['Course'])) {
  ...
  if ($getcourse = $_GET['Course'] > 0) {
    $query = "SELECT a.*, b.fullname FROM {shorturldemux_courses} a LEFT JOIN {course} b ON a.course_id=b.id WHERE a.course_id = :course_id";
    $course = $DB->get_records_sql($query, ["course_id" => (int)$_GET['Course']]);
  ...
  if ($separate2[0] == 0) {
    ...
    foreach ($_GET as $key => $val) {
      ...
      if (strpos($key, '_') != false && empty($_GET[$key]) != true && $key != "Save") {
        $search = '_';
        $replace = '';
        $string_ = str_replace($search, $replace, $key);
        $insert->id = $string_;
      }
      // if short has been changed
      if (strpos($key, '_') == false && empty($_GET[$key]) != true && $key != "Save") {
